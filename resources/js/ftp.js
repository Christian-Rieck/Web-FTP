    	...
    	var _ = {
    		calc_position: function(popover, position) {
    			...
    			if(position == 'top') {
    				...
    				y1 = coordinates.top + $anchor.outerHeight() / 2 - el.outerHeight() / 2;
    				x1 = coordinates.left	- el.outerWidth() - 4;
    			} else {
    				//bottom
    			...
    			return this.each(function() {
    				...
    				if(!data) {
    					var popover = $('<div class="popover" />')
    						...
    						.append('<div class="arrow right-inner-arrow" />')
    		...
    		show: function(ms) {
    			return this.each(function() {
    				...
    				if(data) {
    					...
    					var options = data.options;
    					$this.popover('reposition');
    					popover.clearQueue().css({ zIndex: 950 }).fadeIn(ms ? ms : options.fadeSpeed);
    				}
    			});
    		},
    		/**
