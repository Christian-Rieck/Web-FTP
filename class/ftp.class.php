...
class FTP
{
	...
	*/
	...
	{
		$config = array( "host" => $_SESSION['domain'],
				 "port"     => $_SESSION['port'],
		 	  	 "username" => $_SESSION['username'],
		 		 "password" => $_SESSION['password']);
		 		
		if($_SESSION['ssl'] == TRUE)
		{
	...
	public function download( $remotefile, $localfile, $mode = 'auto', $position = null )
	{
		...
		{
			$transfermode = self::transferMode( $mode, $remotefile );
	...
	public function chmod( $file, $cm )
	{
		if (is_resource( $this->cid ))
		{
			return ftp_chmod( $this->cid, $cm, $file );
		}
	}
		...
		if (is_array( $rawfilelist ))
		{
			foreach ($rawfilelist as $rawfile)
			{
					...
					$fileinfo[8] != '..')
				{
					...
					$i = 9;					
					while(!empty($fileinfo[$i]))
					{
						$file['name'] .= " " . $fileinfo[$i];
						$i++;
					}
	 ...
	 */
	protected function transferMode( $mode, $remotefile )
	{
